...
def calculate_risk():
    ...
    btp = (data["v"] - btp1) / (btpstd)
    # btp = (data["v"] - log_reg0) / (log_reg0)
    btp = (btp - 1 * btp.min()) / (btp.max() - 1 * btp.min()) + 0.27
    risk = (btp)
...
def plot_the_shit():
    ...
    # ax4 = ax1.twinx()
    # ax4.plot(data["t"], OBV, zorder=1, label='stfd', color=color4)

    # ax5 = ax1.twinx()
    # ax5.plot(data["t"], MAD, zorder=0, label='sopr', color=color5)

    # ax6 = ax1.twinx()
    # ax6.plot(data["t"], MA0, zorder=0, label='sopr', color=color6)

    # ax7 = ax1.twinx()
    # ax7.plot(data["t"], vol, zorder=0, label='sopr', color=color7)
